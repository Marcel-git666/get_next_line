...
/*   Updated: 2024/10/02 10:08:10 by mmravec          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line_bonus.h"

char	*append_buffer(char *main_buffer, char *read_buffer);
...
char	*read_from_file(char *main_buffer, int fd)
{
	...
	while (bytes_read > 0)
	{
		...
		if (bytes_read == -1)
			return (free(main_buffer), free(local_buffer), NULL);
	...
	return (free(local_buffer), main_buffer);
}
